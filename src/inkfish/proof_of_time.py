...
def check_proof_of_time_nwesolowski(
        discriminant, x, proof_blob, iterations, int_size_bits, recursion):
    int_size = (int_size_bits + 16) >> 4
    ...
    return check_proof_of_time_nwesolowski_inner(
        discriminant, x, new_proof_blob, iterations, int_size_bits, iter_list, recursion)


def check_proof_of_time_nwesolowski_inner(
        discriminant, x, proof_blob, iterations, int_size_bits, iter_list, recursion):
    """
    ...
    try:
        if len(proof) == 1:
            ...
            return ver_outer and check_proof_of_time_nwesolowski_inner(
                discriminant, proof[-2], serialize_proof([y] + proof[:-2]),
